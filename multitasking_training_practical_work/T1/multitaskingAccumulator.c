...
要求 1：多任务累加器（MultitaskingAccumulator）必须获取四个输入，并在获取一个输入后立即输出输入的累加结果。要求 1：多任务累加器必须获取四个输入，并在获取一个输入后立即输出输入的累加，累加相当于生成一个 256 个整数的数组，其中第 i 个元素是输入数组中第 i 个元素的和。

要求 3：多任务累加器必须保证输入数据在求和之前是正确的。
